   ...
   SubShader {
      Pass {      
         ...
         float4 frag(vertexOutput input) : COLOR {
            float4 encodedNormal = tex2D(_BumpMap, _BumpMap_ST.xy * input.TextureCoordinate.xy + _BumpMap_ST.zw);
            ...
            // default: unlit 
            float3 fragmentColor = _UnlitColor.rgb * tex2D(_MainTex, input.TextureCoordinate);
 
            // low priority: diffuse illumination
            if (attenuation * max(0.0, dot(normalDirection, lightDirection)) >= _DiffuseThreshold) {
               fragmentColor = _LightColor0.rgb * _Color.rgb * tex2D(_MainTex, input.TextureCoordinate); 
            }
            ...
            if (dot(viewDirection, normalDirection) < lerp(_UnlitOutlineThickness, _LitOutlineThickness, max(0.0, dot(normalDirection, lightDirection)))) {
               fragmentColor = _LightColor0.rgb * _OutlineColor.rgb * tex2D(_MainTex, input.TextureCoordinate); 
            }
            ...
            if (dot(normalDirection, lightDirection) > 0.0 && attenuation *  pow(max(0.0, dot(reflect(-lightDirection, normalDirection), viewDirection)), _Shininess) > 0.5) {
               ...
               fragmentColor = _SpecColor.a * _LightColor0.rgb * _SpecColor.rgb + (1.0 - _SpecColor.a) * fragmentColor * tex2D(_MainTex, input.TextureCoordinate);
            }
      ...
      Pass {      
         ...
         Blend SrcAlpha OneMinusSrcAlpha // blend specular highlights over framebuffer
         ...
         float4 frag(vertexOutput input) : COLOR {
            ...
            float3 viewDirection = normalize(_WorldSpaceCameraPos - input.posWorld.rgb); // input.posWorld.xyz?
            float3 lightDirection;
            float attenuation;
            ...
            float4 fragmentColor = tex2D(_MainTex, input.TextureCoordinate);
            if (dot(normalDirection, lightDirection) > 0.0 && attenuation *  pow(max(0.0, dot(reflect(-lightDirection, normalDirection), viewDirection)), _Shininess) > 0.5) {
               // light source on the right side?
               // more than half highlight intensity? 
               fragmentColor = float4(_LightColor0.rgb, 1.0) * _SpecColor * tex2D(_MainTex, input.TextureCoordinate);
            }
